...
## 附錄 D：P3 回報引出的規格補充（2026-09-10）

Codex 交付 P3 時提出 26 個問題。**第 1 條（「新站沒有 shopping client、規則全擋」）經查不成立** —— `db.ts` 有 client，`firestore.rules` 也早有 `match /tools/{toolId}` 且限定 `toolId == 'shopping'`，比要求的更嚴格。它記錯了自己寫過的東西。其餘裁決如下。

### D1 文字版輸出的完整定義
- **精簡版 = 時間＋標題**，但「含地圖連結」是獨立開關，兩者可並存（選精簡＋含地圖就輸出時間、標題、地圖）
- **完整版**輸出：描述、當地語言名、地圖連結、⚠️ 提示。**其餘積木特有欄位（票價、訂位、營業時間、交通方式…）一律不輸出** —— 那是給領隊排程用的，不是給家人看的
- **分隊積木在精簡版也要展開**。它是「誰要去哪」的資訊，縮成一行等於沒講
- **關閉 emoji 時**清除整份輸出的 emoji，包含領隊自己寫在標題與描述裡的。理由是這個開關的用途就是「長輩手機顯示會亂」，只清系統加的沒有意義

### D2 `GuideItem` 補兩個欄位（P4 開頭要做）
P3 發現兩個 schema 缺口，實作只能繞道：
- **`emoji?: string`** —— 舊資料本來就有 emoji，P3 只能塞進 `name`，但那會污染 CSV 更新與跨行程重名的 `name` 比對
- **`order?: number`** —— SPEC 要求「排序永遠依領隊自己排的順序」，但沒有可排序欄位，P3 只能靠陣列順序

補上後，`name` 回歸純品名。

### D3 名稱比對一律正規化
CSV 更新比對與跨行程重名判斷，一律先做：去頭尾空白 → 全形轉半形 → 轉小寫 → 去除 emoji。避免「Anua 魚腥草」與「anua　魚腥草」被當成兩件。

### D4 CSV 的細部語意
- **空欄**：更新既有項目時保留舊值；新增時存空值。**不用空欄清除資料**
- **同一份 CSV 出現重複 `name`**：後列覆蓋前列，並在預覽時標示「第 N 列與第 M 列同名」
- **`imageUrl`**：每列一張。更新時沒填就保留既有圖片，填了就取代整個 `images`
- **不完整組合**：`priceAmount` 沒有 `priceCurrency` → 用行程的 `meta.currency`；有 affiliate URL 沒有 platform → 該列報錯（平台影響揭露文字）

### D5 跨行程複製要複製什麼
複製品名、分類、標籤、備註、描述、圖片、價格、affiliate。**產生新 ID**、`source` 設為 `guide`、**不複製已買勾選狀態**（那是上一趟的事）。

### D6 圖片的生命週期
- 壓縮：依序降品質再降尺寸，仍超過 200KB 就**拒絕上傳並提示**，不要默默存大檔
- **刪除**：移除指南項目或換圖時，只移除 trip document 內的 URL，**不刪 Storage object**。`storage.rules` 也不開 delete
- 代價是會留孤兒檔案。以 5GB 免費額度與這個用量，清理的複雜度不值得。**P5 再評估要不要做清理工具**
- 檔名 `{itemId}-{timestamp}.webp`，用時間戳避免序號碰撞

### D7 affiliate 的揭露文字不信任資料
`GuideItem.affiliate.disclosure` 這個欄位**畫面不讀它**。揭露文字是程式內的固定常數，資料被改也不影響合規。該欄位保留只為記錄來源，不參與渲染。

### D8 affiliate 的範圍分期
SPEC §7.5 提到景點／活動可掛 Klook／KKday，但 P3 只做 `GuideItem`。**積木上的 affiliate 編輯介面延到 P5**；在那之前積木只顯示既有的 affiliate，不提供編輯。

### D9 家庭範本的 schema
範本名用 `Block.title`。保留除 `id` / `dayId` / `order` / `time` 以外的所有欄位。**加入當天時時間依 C1 重新推算**，不沿用範本存下來的時間。

### D10 建立行程的 ID 與預設值
- **trip ID**：城市的 ASCII slug ＋ 年份（例 `busan2026`）。中文城市無法轉 ASCII 時退回 `trip-<年份>`；碰撞就加數字尾碼
- `subtitle` 與 `emergency` 留空由領隊自己補
- `status` 依起訖日對今天計算，不用 `draft`
- **「從既有行程複製結構」是重新生成 `days[]`**：ID、日期、weekday 全部依新起訖日重算，只按索引沿用 `theme` 與 `highlights`；新行程比較長時多出來的天用預設主題

### D11 `tools/shopping` 的 `checked` 值
存 **`'✓'`**（單一字串常數），不存人名或 emoji —— 新站沒有舊站那個「先揀返你係邊個」的身分選擇器，存不了「誰勾的」。刪除 GuideItem 時**不清理** `checked` 的孤兒 key（無害，且清理需要額外一次寫入）。

### D12 trip document 沒有欄位白名單，這是刻意的
P3 派工單寫「保持欄位白名單」，但 `families/{fid}/trips/{tid}` 的 create/update 規則只檢查領隊身分與 `version` 遞增。**這是刻意的**：整份行程存一個 doc，欄位會隨每期增加，在規則裡維護完整 schema 白名單的成本遠高於收益，而寫入者已經是通過認證的領隊。子集合與其他路徑才用白名單。

### D13 iOS 剪貼簿不能用模擬器驗收 ★
P3 派工單的驗收第 1 條寫「在手機上按複製、貼進輸入框」，但 Firebase Emulator 與桌面 375px 版面**都證明不了** iOS Safari 的 user-gesture 限制沒有被打斷。

**這條驗收改成人工項目，由 SS 在 iPhone Safari 實機做一次**，不要求 Codex 交付。桌面 375px 只驗版面，不等於驗剪貼簿。
